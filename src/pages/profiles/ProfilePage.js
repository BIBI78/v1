...
function ProfilePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [{ data: pageProfile }, { data: profileBeats }] =
          await Promise.all([
            axiosReq.get(`/profiles/${id}/`),
            axiosReq.get(`/beats/?owner__profile=${id}`),
          ]);
  ...
  const mainProfileBeats = (
    <>
      <hr />
      <p className="text-center">{profile?.owner}'s beats</p>
      <hr />
      {profileBeats.results.length ? (
        <InfiniteScroll
          ...
          hasMore={!!profileBeats.next}
          next={() => fetchMoreData(profileBeats, setProfileBeats)}
        />
      ) : (
        <Asset
          src={NoResults}
